...
def updateActivos(id):
    ...
    if len(data):
        ...
        for item in data:
            if item["id"] == id:
                continuarActualizar = True  # Inicializar aquí
                                ...
                                while True:
                                    try:
                                        ...
                                        if (r'^[a-zA-Z]+$', NombreEmpresaResponsable) is not None:
                                                    data[0]["EmpresaResponsable"] = NombreEmpresaResponsable
                                                    break
                                        else:
                                ...
                                while True:
                                    try:
                                        ...
                                        if categoria_id in categorias:
                                            data[0]['Categoria']=categorias[categoria_id]
                                ...
                                while True:
                                    try:
                                        ...
                                        if estado_id in Estados:
                                            data[0]['Estado']=Estados[estado_id]
    ...
    #Finalizamos los filtros                            
        ...
        peticion = requests.put(f"http://154.38.171.54:5502/activos/{id}", headers=headers, data=json.dumps(data))
        res = peticion.json()
        return [res]
    else:
        return[{
